...
def main():
    ...
    c = cost_audit()
    ok = (
        sum(a["missing"].values()) == 0
        and a["duplicate_dates"] == 0
        and a["bad_ohlc"] == 0
        and a["monotonic_dates"]
        and a["rows"] > 500
    )
    ...
    out = walk_forward(
        ...
        test=120,
        embargo=5,
        buy_cost_bps=c["buy_cost_bps"],
        sell_cost_bps=c["sell_cost_bps"],
    )
    print("OOS_RESULTS")
    ...
    print(
        ...
        {
            "folds": len(out),
            "mean_return": round(float(out.oos_return.mean()), 6),
            "mean_sharpe": round(float(out.oos_sharpe.mean()), 4),
            "worst_drawdown": round(float(out.oos_max_drawdown.min()), 6),
